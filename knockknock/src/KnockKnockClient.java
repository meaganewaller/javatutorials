...
// Implements the client program that speaks to the KnockKnockServer.
...
public class KnockKnockClient {
    public static void main(String[] args) throws IOException {
        ...
        // When creating its socket, the KnockKnockClient uses the host name of the first CL argument, the name of the comp on your network that is running the server program KnockKnockServer
        // The KnockKnockClient example uses the second command-line argument as the port number when creating its socket -- this is the remote port number, the number of a port on the server computer and is the port to which KnockKnockServer is listening
        // The following command runs the KnockKnockClient with knockknockserver.example.com as the name of the computer that is running the server program KnockKnockServer and 4444 as the remote port number
